...
class MemberGeniusAdmin {
					...
					<tbody>
						...
						<tr>
							<td colspan="4" align="center">
								<input name="miembropress_action_password" type="submit" class="button" value="Send Reset Password Link to Member" />
								...
								<input name="miembropress_action_add" type="submit" class="button" value="Add to Level" />
								<input name="miembropress_action_move" type="submit" class="button" value="Move to Level" />
								<input name="miembropress_action_remove" type="submit" class="button" value="Remove from Level" />
								<input name="miembropress_action_cancel" type="submit" class="button" value="Cancel from Level" />
								<input name="miembropress_action_uncancel" type="submit" class="button" value="Uncancel from Level" />
								<input name="miembropress_action_delete" type="submit" class="button-primary" style="background-color:red !important;" value="Delete Member" onclick="return confirm('Are you SURE you want to delete this member? This action cannot be undone. Press OK to continue, Cancel to stop.');" /><br /><br />
								...
								?>
								<input name="miembropress_action_impersonate" type="submit" class="button" value="Login As <?php echo htmlentities($loginAs); ?>" onmouseover="this.value='Login As <?php echo htmlentities($loginAs); ?> (will log you out of this account)'" onmouseout="this.value='Login As <?php echo htmlentities($loginAs); ?>'" />
							</td>
						<tr>
					</tbody>
				</table>
	...
	public function meta($fullsize=true) {
		...
		<?php endif; ?>
        <label style="display:inline;"><input type="radio" name="miembropress_protect" <?php checked($protected); ?> value="1"> <b>Members Only</b></label><br />
        <label style="display:inline;"><input type="radio" name="miembropress_protect" <?php checked(!$protected); ?>value="0"> <b>Everyone</b></label><br />
        <!-- <label style="display:inline;"><input type="radio" name="miembropress_protect" <?php checked(!$protected); ?>value="-1"> <b>Logged Out Users Only</b></label> -->
        <?php if ($fullsize): ?></blockquote>
		<?php endif; ?>
		<?php if ($fullsize): ?></div>
		<?php endif; ?>
	...
	public function menu_settings() {
		...
		<div class="wrap" style="clear:both; width:1000px;">
			...
			<form method="POST">
				...
				<p><label><input type="checkbox" name="miembropress_settings_notify" <?php checked($notify); ?> /> Email the site administrator <code><?php echo htmlentities(get_option("admin_email")); ?></code> about every transaction (including refunds) sent to this membership site?</label></p>
				<p><label><input type="checkbox" name="miembropress_settings_profile" <?php checked($profiles); ?> /> Allow users to view and edit <a target="_blank" href="<?php echo get_edit_user_link(); ?>">their own profile information</a>?</label></p>
				<h3>Front Page</h3>
				<blockquote>
				...
				<blockquote>
					...
					<p><label>Support Desk URL: <input type="text" name="miembropress_settings_support" value="<?php echo htmlentities($support); ?>" class="code" size="35" /></label> <small>(must include &quot;http://&quot; in web address)</small></p>
		...
		<div class="wrap" style="clear:both; width:1000px;">
			...
			<form method="POST">
				...
				<blockquote>
					...
					<textarea name="miembropress_settings_footer" class="code" cols="90" rows="7" style="font-size:11px;"><?php echo htmlentities($footer); ?></textarea></p>
				</blockquote>
				<p><input type="submit" class="button-primary" value="Save All Changes" /></p>
			</form>
		</div>
	...
	public function menu_content() {
		...
		<div class="wrap" style="clear:both;">
			...
			<form method="post">
				...
				<?php if ($currentLevelName): ?><p><a href="<?php echo admin_url("post-new.php?post_type=" . (($action == "posts") ? "post" : "page") . "&miembropress_new=" . $currentLevel); ?>" class="add-new-h2" style="top:0px;">Add New <?php echo (($action == "posts") ? 'Post' : 'Page'); ?> on &quot;<?php echo $currentLevelName; ?>&quot; Level</a></p><?php endif; ?>
	...
	public function menu_levels_list() {
		...
		<form method="post">
				...
				<tbody>
					...
					<?php foreach ($miembropress->model->getLevels() as $level): ?>
						...
						<tr>
							...
							<td style="padding:5px; vertical-align:middle;"><a href="<?php echo $miembropress->model->signupURL($level->level_hash); ?>" target="_blank"><?php echo $miembropress->model->signupURL($level->level_hash); ?></a></td>
							<td align="left">
								<nobr>
									<p>
										<label>
			...
			<p><input class="button-primary" type="submit" value="Save All Changes" onclick="return miembropress_confirmLevels();" /> <input class="button" type="submit" value="Delete Selected Levels" /></p>
		</form>
	    <script type="text/javascript">
	    <!--
	    jQuery(function() {
	...
	private function menu_members_export() { global $miembropress; $levels = $miembropress->model->getLevels(); ?>
	...
	private function menu_members_list($message=null, $messageLevels=null) {
		...
		<form method="POST">
			...
			<p align="center">
				...
				<?php endif; ?>
				<input type="submit" class="button" value="Search">
				<a title="Add New Member" href="#TB_inline?height=450&amp;width=500&amp;inlineId=miembropress_newmember" class="add-new-h2 thickbox" style="top:-1px;">Add New Member</a>
			</p>
			<p>
				<b>Browse Level:</b>
			...
			<p>Check the users you want to change and make your selection:<br />
				...
				<input name="miembropress_action_cancel" type="submit" class="button" value="Cancel from Level" />
				<input name="miembropress_action_uncancel" type="submit" class="button" value="Uncancel from Level" />
				<input name="miembropress_action_delete" type="submit" class="button-primary" value="Delete Members" onclick="return miembropress_confirm();" />
				</nobr>
			</p>
			<?php endif; ?>
	...
	private function menu_members_temp() {
		...
		<form method="POST">
			...
			<p><input name="miembropress_temps_complete" type="submit" class="button-primary" value="Complete Selected Registrations" />
			<input name="miembropress_temps_delete" type="submit" class="button" value="Delete Selected Registrations" onclick="return confirm('Are You SURE You Want to Delete the Selected Incomplete Registrations? Click OK to delete, Cancel to stop.');" /></p>
		</form>
		<?php
	}
