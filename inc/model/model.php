...
class MiembroPressModel {
	...
	function install() {
		...
		if ($wpdb->get_var("SHOW TABLES LIKE {$this->levelTable}") != $this->levelTable) {
			dbDelta("CREATE TABLE IF NOT EXISTS {$this->levelTable} (`ID` int(11) NOT NULL AUTO_INCREMENT, `level_name` varchar(64) NOT NULL, `level_hash` varchar(6) NOT NULL, `level_all` TINYINT(1) NOT NULL DEFAULT '1', `gdpr_active` TINYINT(1) NOT NULL DEFAULT '0', `gdpr_label` VARCHAR(255) NOT NULL DEFAULT 'He leído y acepto los', `gdpr_url` varchar(254) NOT NULL DEFAULT 'https://miembropress.com', `gdpr_text` varchar(254) NOT NULL DEFAULT 'términos y condiciones', `gdpr_color` varchar(10) NOT NULL DEFAULT '#333', `gdpr_size` int(10) NOT NULL DEFAULT '14', `level_comments` tinyint(1) NOT NULL DEFAULT '1', `level_page_register` int(11) DEFAULT NULL, `level_page_login` int(11) DEFAULT NULL, `level_expiration` int(11) DEFAULT NULL, PRIMARY KEY (`ID`), UNIQUE KEY `level_hash` (`level_hash`), UNIQUE KEY `level_name` (`level_name`), KEY `level_expiration` (`level_expiration`)) DEFAULT CHARSET=utf8;"); 
			$this->cleanup(); 
		}
